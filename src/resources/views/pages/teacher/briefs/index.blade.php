...
<div class="space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-1000">
    ...
    <header class="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <a href="{{ route('teacher.briefs.create') }}" class="flex items-center gap-3 px-8 py-5 indigo-gradient text-white font-black rounded-[2.2rem] shadow-2xl shadow-indigo-500/40 hover:-translate-y-1 transition-all duration-300 group">
    ...
    <div class="grid grid-cols-1 gap-6">
        ...
        <div class="bg-white/80 backdrop-blur-3xl rounded-[3rem] p-8 border border-white shadow-[0_20px_50px_-15px_rgba(0,0,0,0.03)] group hover:shadow-xl transition-all duration-500 flex flex-col lg:flex-row items-center gap-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-8 px-8 border-l border-r border-slate-100 hidden md:grid">
                ...
                <div class="text-center">
                    ...
                    <p class="text-sm font-bold text-slate-700 truncate max-w-[120px]">{{ $brief['sprint']['title'] }}</p>
                </div>
                <div class="text-center">
                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Competencies</p>
                    <p class="text-sm font-bold text-indigo-600">
                        <span class="bg-indigo-50 px-2 py-0.5 rounded-lg">{{ $brief->skills->count() }} Skills</span>
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col gap-2">
                    <form action="{{ route('teacher.briefs.destroy', $brief) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this brief?');">
                        @csrf
                        @method('DELETE')
                        <input type="hidden" name="id" value="{{ $brief['id'] }}">

                        <button type="submit" class="w-10 h-10 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-slate-400 hover:text-rose-600 hover:bg-rose-50/50 hover:shadow-md transition-all group">
                            <span class="material-symbols-outlined text-xl group-hover:scale-110 transition-transform">delete</span>
                        </button>
                    </form>
                </div>
            </div>
        ...
        <div class="py-32 flex flex-col items-center justify-center text-center space-y-6 bg-white/30 backdrop-blur-xl rounded-[4rem] border-2 border-dashed border-slate-200">
            ...
            <a href="{{ route('teacher.briefs.create') }}" class="px-10 py-4 indigo-gradient text-white font-black rounded-full text-xs uppercase tracking-widest shadow-xl shadow-indigo-500/20 hover:scale-105 transition-all">
